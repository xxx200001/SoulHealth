<template>
  <div class="page stack fade-in">
    ...
    <template v-else>
      ...
      <!-- 证型倾向预估（全部填写完毕后展示） -->
      <div v-if="allAnswered && preview.length" class="card">
        <div class="card-title"><span class="dot"></span>问诊证型倾向预估</div>
        <p class="tiny" style="margin:6px 0 var(--sp-3)">
          这是按全部问诊项目的加权预估，仅供参考；正式辨证会一并计入舌象、面象与化验指标。
        </p>
        <div class="stack-sm">
...
const allAnswered = computed(() => dimensions.value.length > 0 && dimensions.value.every((d) => answers[d.key] != null))

function optionsOf(d) { return d.options || SUBJECTIVE }
function isDone(c) { return c.items.length > 0 && c.items.every((d) => answers[d.key] != null) }
