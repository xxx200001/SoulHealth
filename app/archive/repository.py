...
def update_patient(pid: str, **fields) -> None:
    allowed = {"sex", "age_years", "height_cm", "weight_kg", "pseudonym", "name",
              "id_last4", "owner_id", "pregnant", "allergies", "drugs"}
    updates = {k: v for k, v in fields.items() if k in allowed and v is not None}
    # 过敏源与在服西药存 JSON 列（辨证组方的安全闸要用）
    if "allergies" in updates:
        updates["allergies_json"] = json.dumps(list(updates.pop("allergies")),
                                               ensure_ascii=False)
    if "drugs" in updates:
        updates["drugs_json"] = json.dumps(list(updates.pop("drugs")),
                                           ensure_ascii=False)
    if "pregnant" in updates:
        updates["pregnant"] = 1 if updates["pregnant"] else 0
    if not updates:
        return
    if "name" in updates:
...
def get_patient(pid: str) -> Optional[dict]:
    ...
    if not row:
        return None
    d = dict(row)
    d["allergies"] = json.loads(d.pop("allergies_json", None) or "[]")
    d["drugs"] = json.loads(d.pop("drugs_json", None) or "[]")
    d["pregnant"] = bool(d.get("pregnant"))
    return d


def patient_for_tcm(pid: str) -> dict:
    """档案基础信息 → 中医引擎的 patient 入参（键名对齐 dosage/toxicology）。"""
...
def delete_patient(pid: str) -> None:
    ...
    with db.get_conn() as conn:
        for sql in (
            ...
            "DELETE FROM patient_impressions WHERE patient_id=?",
            "DELETE FROM tcm_exams WHERE patient_id=?",
            "DELETE FROM tcm_inquiries WHERE patient_id=?",
...
def snapshot(pid: str) -> dict:
    ...
    return {
        ...
        # 中医四诊：与化验指标同级的档案数据，Agent 与问答都能读到
        "tcm_exams": {"tongue": latest_exam(pid, "tongue"),
                      "face": latest_exam(pid, "face")},
        "tcm_inquiry": latest_inquiry(pid),
        "generated_at": _now(),
    }
    ...
    with db.get_conn() as conn:
        conn.execute(
            ...
            (aid, pid,
             ...
             json.dumps(tcm, ensure_ascii=False) if tcm is not None else None,
             status, _now()),
        )
    touch(pid)
    return aid
...
def get_analysis(aid: str) -> Optional[dict]:
    ...
    for key in ("input_snapshot_json", "risk_tags_json", "mechanism_chain_json",
                ...
                "interpretation_json", "tcm_json", "trace_json"):
...
# ---------------------------------------------------------------- 中医四诊采集
# 舌象/面象量化结果与问诊作答，都是档案的一类数据，和化验指标一样按时间留档，
# 分析时取最近一次。不再像原版那样只存在浏览器 localStorage 里。

def save_tcm_exam(pid: str, exam_type: str, features: dict,
                  quantified: Optional[dict] = None,
                  quality: Optional[dict] = None,
                  image_path: Optional[str] = None,
                  observed_at: Optional[str] = None) -> str:
    if exam_type not in ("tongue", "face"):
        raise ValueError(f"未知的四诊类型: {exam_type}")
    if get_patient(pid) is None:
        raise KeyError(f"患者不存在: {pid}")
    eid = _uid()
    with db.get_conn() as conn:
        conn.execute(
            "INSERT INTO tcm_exams (id, patient_id, exam_type, image_path,"
            " features_json, quantified_json, quality_json, observed_at, created_at)"
            " VALUES (?,?,?,?,?,?,?,?,?)",
            (eid, pid, exam_type, image_path,
             json.dumps(features or {}, ensure_ascii=False),
             json.dumps(quantified or {}, ensure_ascii=False),
...
def _exam_row(row) -> dict:
    ...
    d["quantified"] = json.loads(d.pop("quantified_json") or "{}")
    d["quality"] = json.loads(d.pop("quality_json") or "{}")
    return d


def latest_exam(pid: str, exam_type: str) -> Optional[dict]:
    with db.get_conn() as conn:
        row = conn.execute(
            "SELECT * FROM tcm_exams WHERE patient_id=? AND exam_type=?"
            " ORDER BY observed_at DESC, created_at DESC LIMIT 1",
            (pid, exam_type)).fetchone()
    return _exam_row(row) if row else None


def list_exams(pid: str, exam_type: Optional[str] = None,
               limit: int = 30) -> List[dict]:
    sql = "SELECT * FROM tcm_exams WHERE patient_id=?"
    args: list = [pid]
    if exam_type:
        sql += " AND exam_type=?"
    ...
    with db.get_conn() as conn:
        rows = conn.execute(sql, args).fetchall()
    return [_exam_row(r) for r in rows]


def delete_exam(pid: str, exam_id: str) -> bool:
    with db.get_conn() as conn:
        cur = conn.execute("DELETE FROM tcm_exams WHERE id=? AND patient_id=?",
                           (exam_id, pid))
    return cur.rowcount > 0


def save_tcm_inquiry(pid: str, answers: dict, symptoms: dict,
    ...
    with db.get_conn() as conn:
        conn.execute(
            "INSERT INTO tcm_inquiries (id, patient_id, answers_json,"
            " symptoms_json, drugs_json, allergies_json, observed_at, created_at)"
            " VALUES (?,?,?,?,?,?,?,?)",
            (qid, pid,
             json.dumps(answers or {}, ensure_ascii=False),
             json.dumps(symptoms or {}, ensure_ascii=False),
             json.dumps(list(drugs or []), ensure_ascii=False),
             json.dumps(list(allergies or []), ensure_ascii=False),
             observed_at or _now(), _now()))
    touch(pid)
    return qid


def _inquiry_row(row) -> dict:
    d = dict(row)
    d["answers"] = json.loads(d.pop("answers_json") or "{}")
    d["symptoms"] = json.loads(d.pop("symptoms_json") or "{}")
    d["drugs"] = json.loads(d.pop("drugs_json") or "[]")
    d["allergies"] = json.loads(d.pop("allergies_json") or "[]")
    return d


def latest_inquiry(pid: str) -> Optional[dict]:
    with db.get_conn() as conn:
        row = conn.execute(
            "SELECT * FROM tcm_inquiries WHERE patient_id=?"
            " ORDER BY observed_at DESC, created_at DESC LIMIT 1", (pid,)).fetchone()
    return _inquiry_row(row) if row else None


def list_inquiries(pid: str, limit: int = 30) -> List[dict]:
    with db.get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM tcm_inquiries WHERE patient_id=?"
            " ORDER BY observed_at DESC, created_at DESC LIMIT ?",
            (pid, limit)).fetchall()
    return [_inquiry_row(r) for r in rows]


def collection_status(pid: str) -> dict:
    """四诊采集完成度——首页进度条与"能否出方"判定的唯一数据源。"""
    p = get_patient(pid) or {}
    inq = latest_inquiry(pid)
    with db.get_conn() as conn:
        obs_n = conn.execute(
            "SELECT COUNT(*) FROM observations WHERE patient_id=?", (pid,)).fetchone()[0]
    profile_done = bool(p.get("age_years") and p.get("weight_kg") and p.get("height_cm"))
    symptoms_done = bool((inq or {}).get("symptoms"))
    return {
        "profile": profile_done,
        "labs": obs_n > 0,
        "tongue": latest_exam(pid, "tongue") is not None,
        "face": latest_exam(pid, "face") is not None,
        "inquiry": symptoms_done,
        "observation_count": obs_n,
        # 出方最低条件：基础体征（推算克重要用）+ 症状问诊（辨证证据）
        "ready_for_analysis": profile_done and symptoms_done,
    }
