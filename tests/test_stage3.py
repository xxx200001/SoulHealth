...
def main() -> None:
    ...
    # ---- 报告产物与合规红线
    # 本画像只有化验与影像、无舌面诊问诊，中医辨证链证据不足 → 不出中医组方报告
    # （见 tcm_report.available）；故仍是现代医学链 + 代茶饮 2 类 × docx/md = 4 个文件。
    ...
    # 融合后编排器在最前面多一个 TCM_SYNDROME 节点（中医辨证链），共八节点
    check("trace 八节点齐备（融合后加 TCM_SYNDROME）",
          [s["step"] for s in result["trace"]] ==
          ["LOAD_SNAPSHOT", "TCM_SYNDROME", "IDENTIFY_RISKS", "MATCH_KNOWLEDGE",
