...
sandbox.isolate()                        # 干净的临时库：既验证建库+迁移，也不碰真实数据
...
def main():
    ...
    # 融合后前端换成 Vue（web/src），原 static/index.html + app.js 已不存在，
    # 这里改为对等地断言同样的能力仍在，而不是断言旧文件里的元素 id。
    web = ROOT / "web" / "src"
    api_js = (web / "api" / "index.js").read_text("utf-8")
    lab = (web / "pages" / "LabPage.vue").read_text("utf-8")
    archive = (web / "pages" / "ArchivePage.vue").read_text("utf-8")
    patient = (web / "pages" / "PatientPage.vue").read_text("utf-8")
    login = (web / "pages" / "LoginPage.vue").read_text("utf-8")
    admin = (web / "pages" / "AdminPage.vue").read_text("utf-8")
    analysis = (web / "pages" / "AnalysisPage.vue").read_text("utf-8")

    check("前端保留图片上传与视觉自检入口（本轮澄清：不移除图片抽取）",
          "uploadDocument" in api_js and "selftestVision" in api_js
          and "type=\"file\"" in lab)
    check("前端同时含身份证后四位输入与手动录入表单（两种数据入口并存）",
          "id_last4" in archive and "addObservations" in lab
          and "addFinding" in patient)
    check("前端上传后可回看每份资料的抽取结果（识别有误能核对）",
          "getDocument" in api_js and "getDocument" in lab)
    check("前端含登录/注册与管理员用户管理",
          "register" in login and "adminCreateUser" in admin
          and "adminDeleteUser" in admin)
    check("前端 API 封装自动附带 Authorization 头，401 自动登出",
          "headers.Authorization" in api_js and "sh:unauthorized" in api_js)
    check("前端渲染机制解释链与生物计算明细（bio 侧展示不丢）",
          "mechanism_chain" in analysis and "mean_plddt" in analysis
          and "delta_ll" in analysis)
    check("前端渲染代茶饮的目录门禁替换与加减化裁依据",
          "substitutions" in analysis and "modification_log" in analysis)
    check("前端提供一键载入演示患者",
          "seedDemo" in api_js and "loadDemo" in archive)
